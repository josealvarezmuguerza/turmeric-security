...
public class PolicyEnforcementServiceAuthzNegativeTest extends CreateValidatePES{
	...
	private static final String s_PropFilePath = "PolicyEnforcementServiceAuthzNegativeTest.properties";
	private static Class className = PolicyEnforcementServiceAuthzNegativeTest.class;
	...
	public void setUp() throws Exception {
		m_policyEnforcementConsumer = new BasePolicyEnforcementServiceConsumer("PolicyEnforcementService","PESTestEnv");
		Service service = m_policyEnforcementConsumer.getService();
		service.setSessionTransportHeader("X-TURMERIC-SECURITY-USERID", "admin");
		service.setSessionTransportHeader("X-TURMERIC-SECURITY-PASSWORD", "admin");
	...
	public PolicyEnforcementServiceAuthzNegativeTest(String policyId, String request_id, String testcaseNumber, String testcaseDesc) {
		...
		InputStream inputStream = className.getResourceAsStream(s_PropFilePath);
			...
			if(!debug_tests.equals("") && !debug_tests.equals("null")){
				...
				for(String s:debug_num){
					...
					if(policyid!=null && testcasenum!=null){
	...
	@Test
	public void testNegativePES() throws Exception {
		...
		try{
			...
			VerifyAccessResponse response = m_policyEnforcementConsumer.verifyAccess(request);
	...
	private void validateOutput(VerifyAccessRequest request, VerifyAccessResponse response, String request_id){
		String ack = reader.getPreEntryValue(request_id, "response_AckValue").trim();
		if(ack!=null){
			if(ack.equals("SUCCESS"))
				assertTrue(response.getAck() == AckValue.SUCCESS);
			else if(ack.equals("FAILURE")){
				assertTrue(response.getAck() == AckValue.FAILURE);
				String em = reader.getPreEntryValue(request_id, "response_ErrorMessage");
				if(em!=null){
					for( ErrorData errorData : response.getErrorMessage().getError() ) {
						System.out.println("The error id is ==> " + errorData.getErrorId());
						System.out.println("The error Message is ==> " + errorData.getMessage());
						assertTrue(errorData.getMessage().contains(em));
					}
				}
			}
		}
