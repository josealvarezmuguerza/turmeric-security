...
public class AuthorizationServiceCacheTestValidation extends
		CreateValidateAuthz {
	...
	public void testValidation_test() throws Exception {
		...
		String token = reader.getPreEntryValue(request_id,
				"request_admintest_token").trim();
		String token_header = reader.getPreEntryValue(request_id,
				"request_admintest_headervalue").trim();
		String calc_sub = reader.getPreEntryValue(request_id,
				"request_CalculatorTestService__getSubtraction_subjectlist");
		String admin_sub = reader.getPreEntryValue(request_id,
				"request_AdminTestItemService_addItem_subjectlist");
		...
		GetAuthzCacheKeysResponse resKeys = m_util
				.invokeAdminService_getAuthzCacheKeys(token, token_header);
		displayResourceDetails(resKeys, "invokeAdminService - test2");
		m_resMap.put(
				encodeSvcOp(AuthorizationServiceTestUtils.SERVICE_NAME,
						AuthorizationServiceTestUtils.OPERATION_NAME), 0L);
		validateAuthzCache(resKeys);

		// invoke AdminTestItemValidation:getAuthzCacheKeys Service and
		// CalculatorTestService::getSubtraction
		m_util.invokeAuthzSvc("CalculatorTestService", "getSubtraction",
				calc_values[0], calc_values[1]);
		resKeys = m_util.invokeAdminService_getAuthzCacheKeys(token,
				token_header);
		displayResourceDetails(resKeys,
				"invoke calculator:addition and Admin Service - test2");
		m_resMap.put(encodeStdSvcOp(), 1L);
		m_resMap.put(encodeSvcOp("CalculatorTestService", "getSubtraction"), 0L);
		validateAuthzCache(resKeys);

		// invoke AdminTestItemValidation:addItem Service and
		// CalculatorTestService::getSubtraction
		m_util.invokeAuthzSvc("CalculatorTestService", "getSubtraction",
				calc_values[0], calc_values[1]);
		m_util.invokeAuthzSvc(AuthorizationServiceTestUtils.SERVICE_NAME,
				"addItem", admin_values[0], admin_values[1]);
		resKeys = m_util.invokeAdminService_getAuthzCacheKeys(token,
				token_header);
		displayResourceDetails(resKeys,
				"invoke calculator:subtraction and Admin Service - test2");
	...
	public void testValidation_poke() throws Exception {
		...
		// invoke AdminTestItemValidation:addItem Service and invoke
		// CalculatorTestService:getSubtraction twice
		m_util.invokeAuthzSvc("CalculatorTestService", "getSubtraction",
				calc_values[0], calc_values[1]); // build
		m_util.invokeAuthzSvc("CalculatorTestService", "getSubtraction",
				calc_values[0], calc_values[1]); // hit
		m_util.invokeAuthzSvc(AuthorizationServiceTestUtils.SERVICE_NAME,
				"addItem", admin_values[0], admin_values[1]); // build
		// invoke AdminTestItemValidation:getAuthzCacheKeys Service

		GetAuthzCacheKeysResponse resKeys = m_util
				.invokeAdminService_getAuthzCacheKeys(token, token_header); // miss/build
		...
		m_resMap.put(encodeStdSvcOp(), 0L); // getAuthzCacheKeys was a miss
											// - lazy per-operation loading,
											// remember?
		m_resMap.put(encodeSvcOp("CalculatorTestService", "getSubtraction"), 1L); // hit,
																					// we
																					// invoked
																					// it
																					// twice
		// For addItem, the user adminwltest1 was authorized by
		// policy AdminTestItemValidation_Authz directly, before SG
		// expansion.
		...
		resKeys = m_util.invokeAdminService_getAuthzCacheKeys(token,
				token_header);
		displayResourceDetails(resKeys,
				"invoke Admin Service after poke - testValidation_poke");
		m_resMap.put(encodeStdSvcOp(), 0L); // cache was rebuilt for this
											// key for the get()
		m_resMap.put(encodeSvcOp("CalculatorTestService", "getSubtraction"),
				null); // no more auto-rebuilding on poke
		validateAuthzCache(resKeys);
	...
	public void testValidation_sg() throws Exception {
		...
		String token_header = reader.getPreEntryValue(request_id,
				"request_admintest_headervalue").trim();
		String service2_sub = reader.getPreEntryValue(request_id,
				"request_Service2__getHeaders_subjectlist");
		String admin_sub = reader.getPreEntryValue(request_id,
				"request_AdminTestItemService_addItem_subjectlist");
		String itemManage_sub = reader.getPreEntryValue(request_id,
				"request_SecurityItemManage__securityPlaceOffer_subjectlist");
		...
		m_util.invokeAuthzSvc(AuthorizationServiceTestUtils.SERVICE_NAME,
				"addItem", admin_values[0], admin_values[1]);
		/*
		 * Policies for Service2.getHeaders: - Role2(DEV:Security_DevIDGroup:__,
		 * DEV:DevIDGroup:__) - CalcSG1(USER:CalcSG1:__)
		 */
		m_util.invokeAuthzSvc("Service2", "getHeaders", service2_values[0],
				service2_values[1]);
		/*
		 * Policies(SubjectGroups...) for
		 * SecurityItemManager.securityPlaceOffer: -
		 * Security_BuyerRole(DEV:Security_DevIDGroup:__) -
		 * Security_BuyerPlaceOfferRole(USER:Security_UserBuyerGroup)
		 */
		m_util.invokeAuthzSvc("SecurityItemManage", "securityPlaceOffer",
				itemManage_values[0], itemManage_values[1]);
		m_util.invokeAuthzSvc("SecurityItemManage", "securityPlaceOffer",
				itemManage_values[0], itemManage_values[1]);
		GetAuthzCacheKeysResponse resKeys = m_util
				.invokeAdminService_getAuthzCacheKeys(token, token_header);
		displayResourceDetails(resKeys, "invoke Admin Service sg - test2");
		m_resMap.put(encodeStdSvcOp(), 0L); // the actual getAuthzCacheKeys
											// invocation builds it,
		m_resMap.put(
				encodeSvcOp(AuthorizationServiceTestUtils.SERVICE_NAME,
						"addItem"), 0L);
		m_resMap.put(encodeSvcOp("SecurityItemManage", "securityPlaceOffer"),
				1L); // called
						// twice
		m_resMap.put(encodeSvcOp("Service2", "getHeaders"), 0L); // called
																	// once
	...
	public void testValidation_sgPoke() throws Exception {
		...
		m_resMap.put(encodeStdSvcOp(), 0L); // built during the direct
											// invocation
		m_resMap.put(encodeSvcOp("SecurityItemManage", "securityPlaceOffer"),
				1L);
		validateAuthzCache(resKeys);
		...
		m_resMap.put(encodeStdSvcOp(), 0L); // rebuilt due to the direct
											// invocation
		m_resMap.put(
				encodeSvcOp(AuthorizationServiceTestUtils.SERVICE_NAME,
						"addItem"), 0L); // rebuilt
	...
	public void testValidation_userNotAuthz() throws Exception {
		...
		String itemManage_sub = reader.getPreEntryValue(request_id,
				"request_SecurityItemManage_securityPlaceOffer_subjectlist");
		String[] itemManage_values = null;
		...
		// invoke AdminTestItemValidation:addItem Service
		// SG- Security_DevIDGroup consists "DocDevName" user.
		authorizeServiceRemote_AuthFail("SecurityItemManage",
				"securityPlaceOffer", itemManage_values[0],
				itemManage_values[1]);
	}
	...
	@Test
	public void testValidation_userNotAuthz_validateCacheHits()
			throws Exception {
		System.out
		...
		try {
			...
			e.printStackTrace();
			assertTrue("Exception " + e + " is not a "
					+ ServiceInvocationRuntimeException.class.getName(),
					e instanceof ServiceInvocationRuntimeException);
			final ErrorMessage errorMessage = ((ServiceInvocationRuntimeException) e)
					.getErrorMessage();
			ErrorData errorData = errorMessage.getError().get(0);
			assertEquals(
					"Unexpected error ID for error: \n"
							+ AuthorizationServiceTestErrorFormatter
									.errorMessageToString(errorMessage),
					ErrorDataCollection.svc_security_authz_failed.getErrorId(),
	...
	public void testUpdateInvokedPolicy_noImpact() {
		...
		String token_header = reader.getPreEntryValue(request_id,
				"request_admintest_headervalue").trim();
		String sg_create = reader.getPreEntryValue(request_id,
				"request_SGInclusion").trim();
		String sg_update = reader.getPreEntryValue(request_id,
				"request_removeSubFromSG").trim();
		String test1ext_customError_sub = reader.getPreEntryValue(request_id,
				"request_test1ext_customError2_subjectlist").trim();
		String test1ext_myTestOperation_sub = reader.getPreEntryValue(
				request_id, "request_test1ext_myTestOperation_subjectlist")
				.trim();
		...
		if (test1ext_myTestOperation_sub != null)
			test1ext_myTestOperation_values = test1ext_myTestOperation_sub
					.split(":");
		else
			fail("invalid parameters specified in properties");
		if (sg_update != null)
		...
		PolicyServiceTestHelper helper = PolicyServiceTestHelper
				.getInstance(false);

		try {
			// Create an Authz policy resource:test1ext operation:-
			// customError2 and myTestOperation , assign SG:
			// Security_UserIDGroup_valid
			helper.createPolicy("AUTHZ", "SuryaTestAuthzPolicy",
					"Validate authz cache feature", resourceMap, null, null,
					subjectGInclusion, null, null, 0, 0, null, null, true);
			// Make a call to test1ext:customError2 with "soasec_user1_test"
			// user
			m_util.invokeAuthzSvc("test1ext", "customError2",
					test1ext_customError_values[0],
					test1ext_customError_values[1]);
			resKeys = m_util.invokeAdminService_getAuthzCacheKeys(token,
					token_header);
			m_resMap.put(encodeSvcOp("test1ext", "customError2"), 0L);
			m_resMap.put(encodeSvcOp("test1ext", "myTestOperation"), null);
			validateAuthzCache(resKeys);
			// update SG: Security_UserIDGroup_valid, remove
			// "soasec_user1_test"
			helper.updateSubjectGroup("SURYA_UserIDGroup_CacheTEST",
					sg_values[0], sg_values[1], UpdateMode.DELETE);
			// Make a call to test1ext:myTestOperation with
			// "soasec_user1_test" user
			List<String> sgNames = m_util.invokeAuthzSvc("test1ext",
					"myTestOperation", test1ext_myTestOperation_values[0],
					test1ext_myTestOperation_values[1]);
			resKeys = m_util.invokeAdminService_getAuthzCacheKeys(token,
					token_header);
			m_resMap.put(encodeSvcOp("test1ext", "customError2"), 0L);
	...
	private void authorizeServiceRemote_AuthFail(String serviceName,
			String opName, String subjectDomain, String subjectName)
			throws ServiceException, MalformedURLException {
		AuthorizeResponseType result = m_util.authorize(serviceName, opName,
				subjectDomain, subjectName);

		assertEquals("ACK value not as expected", AckValue.FAILURE,
				result.getAck());
		assertTrue(result.getErrorMessage() != null);
		System.out.println("failed with error message: \n"
				+ AuthorizationServiceTestErrorFormatter
						.errorMessageToString(result.getErrorMessage()));
	}

	private void displayResourceDetails(GetAuthzCacheKeysResponse resKeys,
			String message) {
		...
		while (itr.hasNext()) {
			...
			System.out.println("resource = "
					+ EncodingUtils.encodeResOpKey(resource.getResOp()
							...
							.getResourceName(), resource.getResOp()
							.getOperationName()) + ", hits = "
					+ resource.getHits());
		}
	...
	private void validateSGStats(List<SGInfoType> resInfoList,
			Map<String, Long> expStats) {
		Map<String, Long> outStats = new HashMap<String, Long>(
				resInfoList.size());
	...
	private void validateResOpStats(List<ResourceInfoType> resInfoList,
			Map<String, Long> expStats) {
		Map<String, Long> outStats = new HashMap<String, Long>(
				resInfoList.size());
		for (ResourceInfoType resInfo : resInfoList) {
			outStats.put(EncodingUtils.encodeResOpKey(resInfo.getResOp()
					.getResourceType(), resInfo.getResOp().getResourceName(),
	...
	private void validateStats(Map<String, Long> expStats,
		...
		for (Map.Entry<String, Long> expEntry : expStats.entrySet()) {
			...
			if (expEntry.getValue() == null && outStats.containsKey(expKey)) {
				fail("Out stats " + outStats + " should not contain key "
						+ expKey);
			} else if (expEntry.getValue() != null
					&& !outStats.containsKey(expKey)) {
				fail("Out stats " + outStats + " don't even contain key "
						+ expKey);
			}
			assertEquals("Out stats " + outStats + " have wrong value for key "
					+ expKey, expEntry.getValue(), outStats.get(expKey));
		}
	}
