...
public class CreateResourceNegativeTests{

	private static final String CONFIG_DELIMITER = ",";
	private String testCaseName;
	private String inputData;
	private String expectedResult;
	private static final String s_PropFilePath = "CreateResourceNegativeTests.properties";
	...
	public CreateResourceNegativeTests(String testCaseName,String inpurData, String expectedResult) {
	...
	public static Collection loadInputData() {
		...
		InputStream inputStream = CreateResourceNegativeTests.class.getResourceAsStream(s_PropFilePath);
		try {
	...
	public void createResources() throws Exception {
		...
		if (inputData.equals("null")) {
			...
			while(resourcesTokens.hasMoreTokens()) {
				constructCreateRequest(resourceList,resourcesTokens.nextToken());
			}
		}
		try {
			...
			if (getExpectedAckValue().equalsIgnoreCase("success")) {
				assertEquals(errorMessage, AckValue.SUCCESS,createResourcesResponse.getAck());
				assertNull(errorMessage, createResourcesResponse.getErrorMessage());	
			} else {
				assertEquals(errorMessage, AckValue.FAILURE,createResourcesResponse.getAck());
				assertNotNull(errorMessage, createResourcesResponse.getErrorMessage());
	...
	private void constructCreateRequest(List<Resource> resourceList, String resourceData) {
		
		String resourceName = null;
		String resourceType = null;
		String resourceDesc = null;
		StringTokenizer resourceTokens = new StringTokenizer(resourceData,CONFIG_DELIMITER);
		...
		System.out.println("\n > Resource Name = "+ resourceName + " ; Type = "
				+ resourceType + " ; Description = "+ resourceDesc);
	...
	private void cleanUpResources(List<Resource> resourceList) {
		for(Resource resource:resourceList) {
			if(resource != null){
				// in the test we do add a null as a resource deliberately. and that fails here
				PolicyServiceTestHelper.getInstance().cleanUpResource(resource.getResourceName(), resource.getResourceType());
			}
		}
		}
